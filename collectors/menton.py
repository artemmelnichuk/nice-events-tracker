"""Menton Riviera & Merveilles Tourist Office event agenda collector.

get-planning/schedule/all-the-diary on menton-riviera-merveilles.co.uk runs
the same "Tourism System" platform as Cannes -- same `item_sheet` listing
cards, same coarse `.item-infos-type` badge, same `criterias-list` /
`localisation-container` on the detail page, all plain server-rendered
HTML (confirmed, unlike Antibes' Angular-rendered detail pages). This
tourist office covers several communes beyond Menton itself (Sospel,
Beausoleil, Roquebrune-Cap-Martin, Castellar, Castillon), so location
comes from the event's own address, not a hardcoded "Menton".

robots.txt disallows `listpage=1` (page one is the bare agenda URL), same
restriction as Cannes. A handful of event slugs with an accented
punctuation mark also 301-redirect to themselves once `requests`
re-encodes the URL for the hop -- same site bug as Cannes, same fix
(cap redirects at 5 instead of the default 30).
"""
...
def parse_locality(event: dict[str, Any]) -> str:
    """The event's commune from its JSON-LD address, defaulting to Menton.

    The tourist office also lists events in nearby communes (Sospel,
    Beausoleil, Roquebrune-Cap-Martin...), so the listing site is not a
    reliable location on its own.
    """
    locality = ((event.get("location") or {}).get("address") or {}).get("addressLocality")
    return locality or "Menton"
...
class MentonCollector(BaseCollector):
    ...
    def _get(self, session: requests.Session, url: str) -> str:
        ...
        # with an accented punctuation mark 301-redirect to themselves once
        # `requests` re-encodes the URL for the hop. Failing fast at 5 avoids
