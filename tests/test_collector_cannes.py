...
from collectors import cannes
from collectors.cannes import CannesCollector, ListingEntry, page_url, parse_detail, parse_listing_card, parse_price, parse_venue
...
def card(sheet_id="1", title="Julien Clerc - Une vie", type="CULTURAL", url=None) -> dict:
    return dict(sheet_id=sheet_id, title=title, type=type, url=url or f"https://en.cannes-france.com/offers/e-{sheet_id}/")


def detail_page(
    criteria=("Cultural", "Concert", "Light music"),
    start="2026-09-20",
    end="2026-09-20",
    status="http://schema.org/EventScheduled",
    location="Palais des Festivals et des Congrès de Cannes - Grand Auditorium, 1 Boulevard de la Croisette, 06400 Cannes",
    rates="Golden Square: €70 1st Series: Full price €60 - Reduced price €54 2nd Series: Full price €50",
    locality="Cannes",
) -> str:
    event = {
        "@type": "Event",
        "name": "x",
        "url": "https://en.cannes-france.com/offers/julien/",
        "startDate": start,
        "endDate": end,
        "eventStatus": status,
...
ENTRY = ListingEntry(sheet_id="1", title="Julien Clerc - Une vie", type="CULTURAL", url="https://en.cannes-france.com/offers/julien/")


class UrlTests(unittest.TestCase):
    def test_page_one_is_the_bare_agenda_url_and_never_listpage_one(self) -> None:
        # robots.txt disallows `listpage=1`.
        self.assertEqual(page_url(1), cannes.BASE_URL)
...
class ListingTests(unittest.TestCase):
    def test_parses_a_listing_card(self) -> None:
        soup = BeautifulSoup(listing_page(card(sheet_id="5986570", title="Julien Clerc - Une vie")), "lxml")

        entry = parse_listing_card(soup.select_one("article.item_sheet"))

        self.assertEqual(entry.sheet_id, "5986570")
        self.assertEqual(entry.title, "Julien Clerc - Une vie")
...
class DetailTests(unittest.TestCase):
    def test_builds_a_concert_record(self) -> None:
        ...
        self.assertEqual(record.source, "cannes")
        self.assertEqual(record.category, "Concert")
        self.assertEqual(record.theme, "Light music")
        self.assertEqual(record.start_date, "2026-09-20")
        self.assertEqual(record.end_date, "2026-09-20")
        self.assertEqual(record.venue, "Palais des Festivals et des Congrès de Cannes - Grand Auditorium")
        self.assertEqual(record.location, "Cannes")
        self.assertEqual(record.price, "€50 - €70")
    ...
    def test_a_multi_day_event_keeps_both_dates(self) -> None:
        record = parse_detail(detail_page(start="2026-09-03", end="2026-09-24"), ENTRY)

        self.assertEqual((record.start_date, record.end_date), ("2026-09-03", "2026-09-24"))
    ...
    def test_an_event_outside_cannes_proper_keeps_its_own_commune(self) -> None:
        # The agenda also lists events in nearby communes, e.g. Mandelieu-la-Napoule.
        record = parse_detail(detail_page(locality="Mandelieu-la-Napoule"), ENTRY)

        self.assertEqual(record.location, "Mandelieu-la-Napoule")

    def test_a_missing_locality_falls_back_to_cannes(self) -> None:
...
class VenueTests(unittest.TestCase):
    ...
    def test_takes_the_place_name_before_the_street(self) -> None:
        self.assertEqual(parse_venue(self._soup("Théâtre Debussy, 1 Boulevard de la Croisette, 06400 Cannes")), "Théâtre Debussy")

    def test_a_bare_street_address_gives_no_venue(self) -> None:
        self.assertEqual(parse_venue(self._soup("12 Rue Meynadier, 06400 Cannes")), "")
...
class PriceTests(unittest.TestCase):
    ...
    def test_freedom_card_is_not_free(self) -> None:
        self.assertEqual(parse_price(self._soup("Freedom card, price on request")), "")
...
class CollectorTests(unittest.TestCase):
    def setUp(self) -> None:
        patcher = mock.patch.object(cannes.time, "sleep")
        patcher.start()
    ...
    def test_collects_concerts_and_only_opens_detail_pages_for_plausible_types(self) -> None:
        ...
        result = CannesCollector().collect(session)
    ...
    def test_an_event_repeated_across_listing_pages_is_fetched_once(self) -> None:
        ...
        CannesCollector().collect(session)

        self.assertEqual(session.requested.count("https://en.cannes-france.com/offers/e-1/"), 1)
    ...
    def test_a_failing_detail_page_is_counted_and_the_rest_still_load(self) -> None:
        ...
        session = FakeSession(pages, failing={"https://en.cannes-france.com/offers/e-1/"})

        result = CannesCollector().collect(session)

        self.assertEqual((result.found, result.errors), (1, 1))
        self.assertIn("Julien Clerc", result.error_messages[0])
    ...
    def test_respects_limit(self) -> None:
        ...
        result = CannesCollector().collect(FakeSession(pages), limit=1)
